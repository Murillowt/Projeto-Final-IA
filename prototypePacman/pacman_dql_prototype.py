...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, action, level_board):
        ...
        # Primeiro, verificamos se o próximo movimento está dentro dos limites do tabuleiro.
        # O túnel gera movimentos para as colunas -1 e 30, que estão fora dos limites normais.
        if 0 <= next_col < 30:
            # Se estiver dentro dos limites, fazemos a verificação de parede normal.
            if level_board[next_row][next_col] < 3:
                self.row, self.col = next_row, next_col
        else:
            # Se estiver fora dos limites (é um movimento de túnel),
            # permitimos o movimento sem verificar a parede.
            self.row, self.col = next_row, next_col
...
class Ghost(pygame.sprite.Sprite):
    ...
    def update(self, player, blinky, powerup_active, level_board, game_turn):
        # --- LÓGICA DE REDUÇÃO DE VELOCIDADE NO TÚNEL ---
        TUNNEL_ROW = 15
        # Define as colunas que são consideradas "dentro do túnel"
        is_in_tunnel = self.row == TUNNEL_ROW and (self.col <= 5 or self.col >= 24)

        # Se o fantasma está no túnel, ele só se move a cada 2 turnos (nos turnos pares)
        if is_in_tunnel and game_turn % 2 != 0:
            return  # Pula o resto da função, fazendo o fantasma ficar parado neste turno
    ...
    def get_valid_moves(self, level_board):
        moves = {}
        TUNNEL_ROW = 15  # Defina aqui a linha correta do túnel

        # --- CIMA (2) E BAIXO (3) - Lógica original mantida ---
        ...
        # --- ESQUERDA (1) - Lógica do túnel adicionada ---
        if self.row == TUNNEL_ROW and self.col == 0:
            moves[1] = (TUNNEL_ROW, -1) # Permite o movimento para "fora do mapa"
        elif self.col > 0 and (level_board[self.row][self.col - 1] < 3 or (self.is_in_box and level_board[self.row][self.col - 1] == 9)):
            moves[1] = (self.row, self.col - 1)

        # --- DIREITA (0) - Lógica do túnel adicionada ---
        if self.row == TUNNEL_ROW and self.col == 29:
            moves[0] = (TUNNEL_ROW, 30) # Permite o movimento para "fora do mapa"
        elif self.col < 29 and (level_board[self.row][self.col + 1] < 3 or (self.is_in_box and level_board[self.row][self.col + 1] == 9)):
            moves[0] = (self.row, self.col + 1)
            
        # --- LÓGICA DE NÃO REVERTER A DIREÇÃO (Mantida) ---
...
MEMORY_SIZE = 40000
LR = 3e-4
model_file = 'dql_prototype_model.pth'

#parametros de reward:
score_multiplier = 2
TURNS_PER_PENALTY_INCREASE = 20
PENALTY_BY_TURNS_WITHOUT_PALLET_MULTIPLIER = 3
NEAR_MISS_THRESHOLD = 2  # Distância em "casas" para considerar perigoso
NEAR_MISS_PENALTY = -3   # A penalidade a ser aplicada
...
# VERSÃO FINAL COM BATCH NORMALIZATION
class DuelingDQN(nn.Module):
    def __init__(self, n_observations, n_actions):
        ...
        # Braço da Vantagem (Advantage Stream)
...
def get_n_nearest_pellets(player, pellet_group, n=5):
    """
    Encontra as 'n' pastilhas mais próximas do jogador e retorna suas posições.
    Garante que a lista retornada sempre tenha tamanho 'n', preenchendo com
    a mais próxima se houver menos de 'n' no mapa.
    """
    if not pellet_group:
        # Retorna 'n' posições nulas se não houver mais pastilhas
        return [(-1, -1)] * n

    pellet_distances = []
    for pellet in pellet_group:
        # Usa a distância ao quadrado para ser mais rápido (evita raiz quadrada)
        dist_sq = (player.row - pellet.rect.centery // TILE_HEIGHT)**2 + (player.col - pellet.rect.centerx // TILE_WIDTH)**2
        pellet_pos = (pellet.rect.centery // TILE_HEIGHT, pellet.rect.centerx // TILE_WIDTH)
        pellet_distances.append((dist_sq, pellet_pos))
    
    # Ordena a lista pela distância (do menor para o maior)
    pellet_distances.sort(key=lambda x: x[0])
    
    # Pega as posições das 'n' pastilhas mais próximas
    nearest_positions = [pos for dist, pos in pellet_distances[:n]]
    
    # Lógica de Preenchimento (Padding)
    while len(nearest_positions) < n:
        nearest_positions.append(nearest_positions[0])
        
    return nearest_positions

def get_state_discrete(player, ghosts, powerup_counter, level_board, pellets_group, power_pellets_group, initial_pellet_count, initial_power_pellet_count, turns_since_pellet):
    features = []
    ...
    for ghost in sorted(ghosts, key=lambda g: g.id):
        ...
        # --- ADIÇÃO DA DIREÇÃO DO FANTASMA (ONE-HOT ENCODING) ---
        # Cria um vetor de 4 posições, uma para cada direção (R, L, U, D)
        ghost_direction_vector = [0.0, 0.0, 0.0, 0.0]
        # Ativa a posição correspondente à direção atual do fantasma
        if ghost.direction < 4: # Garante que a direção seja um índice válido
            ghost_direction_vector[ghost.direction] = 1.0
        features.extend(ghost_direction_vector)
    ...
    # --- ADIÇÃO DAS NOVAS FEATURES DE LOCALIZAÇÃO DE PASTILHAS ---
    # Cria subgrupos para a busca (isso pode ser otimizado se ficar lento)
    ...
    turns_normalized = turns_since_pellet / 100.0
    features.append(turns_normalized)

    # --- ADIÇÃO DAS 5 PASTILHAS MAIS PRÓXIMAS ---
    nearest_pellets_pos = get_n_nearest_pellets(player, pellets_group, 5)

    for r, c in nearest_pellets_pos:
        # Normaliza cada coordenada e adiciona ao estado
        features.append(r / max_row)
        features.append(c / max_col)
    # --- FIM DA ADIÇÃO ---
...
def get_location_type_features(row, col, level_board):
    ...
    if is_corridor_or_corner:
        ...
        if abs(moves[0] - moves[1]) != 2:
            # É um canto, não um corredor reto. Podemos criar uma feature separada se quisermos.
            # Por simplicidade, vamos apenas considerá-lo um "corredor/canto".
            pass
...
def get_valid_moves_for_pos(row, col, level_board):
    """
    Verifica os movimentos válidos a partir de uma posição, 
    incluindo a lógica para os túneis de teletransporte.
    """
    moves = {}
    TUNNEL_ROW = 15

    # --- CIMA (2) E BAIXO (3) ---
    if row > 0 and level_board[row - 1][col] < 3:
        moves[2] = (row - 1, col)
    if row < 30 and level_board[row + 1][col] < 3:
        moves[3] = (row + 1, col)

    # --- ESQUERDA (1) ---
    # Verifica se está na entrada esquerda do túnel
    if row == TUNNEL_ROW and col == 0:
        moves[1] = (TUNNEL_ROW, -1)  # Permite o movimento para "fora do mapa"
    # Senão, verifica o movimento normal para a esquerda
    elif col > 0 and level_board[row][col - 1] < 3:
        moves[1] = (row, col - 1)

    # --- DIREITA (0) ---
    # Verifica se está na entrada direita do túnel
    if row == TUNNEL_ROW and col == 29:
        moves[0] = (TUNNEL_ROW, 30) # Permite o movimento para "fora do mapa"
    # Senão, verifica o movimento normal para a direita
    elif col < 29 and level_board[row][col + 1] < 3:
        moves[0] = (row, col + 1)
...
def choose_action(state, valid_moves, policy_net, steps_done, current_direction):
    ...
    if random.random() > eps_threshold:  # Exploitation
        policy_net.eval() # Coloca a rede em modo de avaliação
        with torch.no_grad():
...
def main():
    ...
    log_file_path = 'prototype_training_log2.csv'
    ...
    for episode in range(nTreino + nRender):
        ...
        # --- NOVA LÓGICA DE CRIAÇÃO DE PASTILHAS ---
        ...
        for turn in range(7500): # Limite de turnos para evitar loops infinitos

            turns_since_pellet += 1
            ...
            state = get_state_discrete(player, ghosts, powerup_counter, level, normal_pellets_group, power_pellets_group, initial_pellet_count, initial_power_pellet_count, turns_since_pellet)
            ...
            if not player_lost_life_this_turn:
                ghosts.update(player, blinky, powerup_counter > 0, level, turn)
                for ghost in ghosts:
            ...
            # Calcula a magnitude da penalidade
            penalty_level =  PENALTY_BY_TURNS_WITHOUT_PALLET_MULTIPLIER * (turns_since_pellet // TURNS_PER_PENALTY_INCREASE)

            if penalty_level > 0:
                reward -= penalty_level
            ...
            if last_action != -1 and action == opposite_actions.get(last_action):
                reward -= 5 # Penalidade por movimento de vai e vem

            # --- Etapa 2.x: Penalidade por "Quase-Morte" (Near Miss Penalty) ---

            # A penalidade só se aplica se o power-up NÃO estiver ativo
            if powerup_counter <= 0:
                for ghost in ghosts:
                    # Verifica se o fantasma é uma ameaça
                    if not ghost.is_dead:
                        # Calcula a distância de Manhattan (soma das diferenças em x e y)
                        # É mais eficiente que a Euclidiana para ambientes em grade.
                        distance = abs(player.row - ghost.row) + abs(player.col - ghost.col)
                        
                        if distance <= NEAR_MISS_THRESHOLD:
                            reward += NEAR_MISS_PENALTY
                            # A penalidade é aplicada para cada fantasma que estiver perto demais,
                            # tornando situações de cerco ainda mais perigosas.
            ...
            if done:
                ...
                next_state = get_state_discrete(player, ghosts, powerup_counter, level, normal_pellets_group, power_pellets_group, initial_pellet_count, initial_power_pellet_count, turns_since_pellet)
