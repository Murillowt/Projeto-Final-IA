...
print(f"Dispositivo sendo usado pelo PyTorch: {device}")
...
EPS_START = 0.8           # Valor inicial do epsilon
EPS_END = 0.05            # Valor final do epsilon
EPS_DECAY = 3000          # Taxa de decaimento do epsilon (controla a velocidade)
...
#Função para utilizar feature engenering:
def get_state_engineered(player_x, player_y, direction, ghosts, powerup, power_counter, level_board):
    """
    Cria uma representação vetorial do estado do jogo usando features escolhidas (engenharia de features).
    """
    features = []
    
    # Normalizadores (para manter os valores em uma escala semelhante, geralmente entre 0 e 1)
    # A maior distância possível no mapa é sqrt(900^2 + 950^2) ~= 1300
    max_dist = 1300.0
    
    # Features 1-4: Distância para cada fantasma e se ele está vulnerável
    for ghost in ghosts:
        dist = math.sqrt((player_x - ghost.x_pos)**2 + (player_y - ghost.y_pos)**2)
        features.append(dist / max_dist) # Distância normalizada
        
        # O fantasma está vulnerável (powerup ativo E não foi comido ainda)?
        is_vulnerable = 1.0 if powerup and not ghost.dead and not eaten_ghost[ghost.id] else 0.0
        features.append(is_vulnerable)

    # Feature 9: Tempo de power-up restante (normalizado)
    features.append(power_counter / 600.0)

    # Feature 10: Distância para a pastilha de poder mais próxima
    min_power_pellet_dist = float('inf')
    found_pellet = False
    for i in range(len(level_board)):
        for j in range(len(level_board[i])):
            if level_board[i][j] == 2: # Se for uma pastilha de poder
                pellet_x, pellet_y = j * 30 + 15, i * 32 + 15
                dist = math.sqrt((player_x - pellet_x)**2 + (player_y - pellet_y)**2)
                if dist < min_power_pellet_dist:
                    min_power_pellet_dist = dist
                    found_pellet = True
    # Se não houver mais pastilhas de poder, use a distância máxima
    features.append((min_power_pellet_dist / max_dist) if found_pellet else 1.0)
    
    # Feature 11: Direção atual do Pac-Man (normalizada)
    features.append(direction / 3.0)

    # Features 12-15: Paredes ao redor do Pac-Man (visão de 1 passo)
    ...
    # Verifica se a próxima posição na direção atual é uma parede
    # Direita
    features.append(1.0 if level_board[center_y // num1][(center_x + num2) // num2] >= 3 else 0.0)
    # Esquerda
    features.append(1.0 if level_board[center_y // num1][(center_x - num2) // num2] >= 3 else 0.0)
    # Cima
    features.append(1.0 if level_board[(center_y - num1) // num1][center_x // num2] >= 3 else 0.0)
    # Baixo
    features.append(1.0 if level_board[(center_y + num1) // num1][center_x // num2] >= 3 else 0.0)
...
while run:
    ...
    if not game_over and not game_won:
        ...
        state = get_state_engineered(player_x, player_y, direction, all_ghosts, powerup, power_counter, level)
        ...
        if done:
            ...
            next_player_x, next_player_y = move_player(player_x, player_y) # Simula o próximo passo para o estado
            next_state = get_state_engineered(next_player_x, next_player_y, direction, all_ghosts, powerup, power_counter, level)
