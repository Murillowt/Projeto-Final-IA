...
epsilon = 1.0  # Probabilidade de exploração inicial
epsilon_min = 0.01
epsilon_decay = 0.9995
q_table_file = 'q_table.pkl'

# Carrega a Q-table se ela já existir, senão, cria uma nova
...
# Variável para contar o número de jogos (episódios)
episode_count = 0
...
def get_state(player_x, player_y, ghosts, powerup_active):
    """
    Cria uma representação discreta do estado do jogo.
    O estado é uma tupla contendo as posições em grid do pacman, dos fantasmas,
    e o status do powerup.
    """
    num1 = ((HEIGHT - 50) // 32)
    num2 = (WIDTH // 30)
    
    # Posição do Pac-Man no grid
    px = int((player_x + 23) // num2)
    py = int((player_y + 24) // num1)

    # Posição dos fantasmas no grid
    ghost_pos = []
    for ghost in ghosts:
    ...
    # O estado é uma tupla para ser usável como chave de dicionário
...
def choose_action(state, turns_allowed):
    ...
    if not valid_actions:
        return direction  # Mantém a direção atual se estiver preso
    ...
    if random.uniform(0, 1) < epsilon:
        ...
        # Explora: escolhe a melhor ação da Q-table
        q_values = q_table.get(state, {})
        # Filtra q_values para incluir apenas ações válidas
        valid_q_values = {action: q_values.get(action, 0.0) for action in valid_actions}
        if not valid_q_values:
            return random.choice(valid_actions) # Se não há conhecimento, age aleatoriamente
...
### Q-LEARNING: FUNÇÃO PARA REINICIAR O JOGO AUTOMATICamente ###
def reiniciar_jogo():
    ...
    #Lógica para decidir se a run será renderizada ou não
    ...
    # Lógica de fim de episódio do Q-Learning
    episode_count += 1
...
while run:
    ...
    if not game_over and not game_won:
        ...
        # 6. Atualizar a Q-Table
        update_q_table(current_state, action, reward, next_state)

    ### Q-LEARNING: LÓGICA DE REINÍCIO AUTOMÁTICO ###
    if game_over or game_won:
        # Adiciona um pequeno delay para que a tela de "Game Over" seja visível
        if game_over:
            if RENDER:
                draw_misc() 
                pygame.display.flip()
