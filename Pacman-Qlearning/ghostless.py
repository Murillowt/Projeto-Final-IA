...
HEIGHT = NUM_ROWS * TILE_H + 50
pygame.init()
screen = pygame.display.set_mode([WIDTH, HEIGHT])
partidas_vencidas = 0

# --- CONSTANTES GLOBAIS ---
pygame.display.set_caption("Pac-Man com Q-Learning (Apenas Inky)")
...
PI = math.pi
# #############################################################################
# ################ ALTERAÇÃO 1: PONTO DE RESPAWN CENTRALIZADO #################
# #############################################################################
# Define as coordenadas de grade (coluna, linha) para o nascimento do Inky
INKY_SPAWN_POS_GRID = (14, 2) 
INKY_SPAWN_POS_PIXELS = (INKY_SPAWN_POS_GRID[0] * TILE_W, INKY_SPAWN_POS_GRID[1] * TILE_H)
INKY_EXIT_POS_PIXELS = (14 * TILE_W, 2 * TILE_H) # Ponto para onde ele vai ao sair da caixa
# #############################################################################
...
class Ghost:
    # (O código da classe Ghost permanece o mesmo)
    ...
    def draw(self):
        # Condição 'eaten_ghost' é importante para o fantasma não ficar vulnerável após respawnar
        # durante o mesmo power-up.
    ...
    def check_collisions(self):
        ...
        if 0 < self.center_x // TILE_W < (NUM_COLS - 1):
            if level[(self.center_y - num3) // TILE_H][self.center_x // TILE_W] == 9: self.turns[2] = True
            if level[self.center_y // TILE_H][(self.center_x - num3) // TILE_W] < 3 or (level[self.center_y // TILE_H][(self.center_x - num3) // TILE_W] == 9 and (self.in_box or self.dead)): self.turns[1] = True
            if level[self.center_y // TILE_H][(self.center_x + num3) // TILE_W] < 3 or (level[self.center_y // TILE_H][(self.center_x + num3) // TILE_W] == 9 and (self.in_box or self.dead)): self.turns[0] = True
            if level[(self.center_y + num3) // TILE_H][self.center_x // TILE_W] < 3 or (level[(self.center_y + num3) // TILE_H][self.center_x // TILE_W] == 9 and (self.in_box or self.dead)): self.turns[3] = True
            if level[(self.center_y - num3) // TILE_H][self.center_x // TILE_W] < 3 or (level[(self.center_y - num3) // TILE_H][self.center_x // TILE_W] == 9 and (self.in_box or self.dead)): self.turns[2] = True
            ...
            if self.direction == 0 or self.direction == 1:
                ...
                if 12 <= self.center_y % TILE_H <= 18:
                    if level[self.center_y // TILE_H][(self.center_x - num3) // TILE_W] < 3: self.turns[1] = True
                    if level[self.center_y // TILE_H][(self.center_x + num3) // TILE_W] < 3: self.turns[0] = True
        else:
            self.turns[0] = True
            self.turns[1] = True
...
# As funções draw_misc, check_collisions, draw_board, draw_player, check_position, move_player
# e get_state permanecem as mesmas.
...
def draw_board(flicker=False):
    for i in range(NUM_ROWS):
        for j in range(NUM_COLS):
            # Lógica de desenho agora usa TILE_W e TILE_H
...
def check_position(centerx, centery):
    ...
    if 0 < centerx // TILE_W < (NUM_COLS - 1):
        # Lógica de checagem de posição agora usa TILE_W e TILE_H
        if direction == 0:
            if level[centery // TILE_H][(centerx - num3) // TILE_W] < 3: turns[1] = True
        if direction == 1:
            if level[centery // TILE_H][(centerx + num3) // TILE_W] < 3: turns[0] = True
        if direction == 2:
            if level[(centery + num3) // TILE_H][centerx // TILE_W] < 3: turns[3] = True
        if direction == 3:
            if level[(centery - num3) // TILE_H][centerx // TILE_W] < 3: turns[2] = True
...
def get_target():
    ...
    # Caso padrão: perseguir o Pac-Man
...
def run_game_step(action):
    ...
    # A recompensa base por passo (punição de tempo para incentivar a rapidez)
    reward = -1 
    
    ### ALTERAÇÃO: Calcular distância ANTES do movimento ###
    ...
    if moving:
        ...
        ### ALTERAÇÃO: Calcular distância DEPOIS do movimento e aplicar recompensa ###
        dist_after_move = get_distance_to_closest_food(player_x, player_y)
        
        # Se a distância para a comida diminuiu, damos uma pequena recompensa.
        if dist_after_move < dist_before_move:
            reward += 1.5
        # Se aumentou, uma pequena punição para desencorajar movimentos ruins.
...
# A função play_game_for_visualization e o Loop Principal de Treinamento
# não precisam de alterações, pois usam as funções que foram corrigidas.
...
if __name__ == '__main__':
    ...
    print("O pacman venceu:  {partidas_vencidas} partidas")
    pygame.quit()